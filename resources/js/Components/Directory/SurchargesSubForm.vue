<template>
    <div class="flex items-center pb-3">
        ...
        <div class="w-3/12 pr-2 relative flex flex-col">
            <input type="number"
                   v-model="surcharge.from"
                   required
        ...
        <div class="w-3/12 px-1 relative flex flex-col">
            <input type="number"
                   v-model="surcharge.to"
                   required
                   placeholder="Сумма до"
                   class="px-3 py-2 rounded-lg border-2 border-gray-150 placeholder-gray-150 focus:ring-0
                                        focus:border-gray-150 focus:outline-none text-gray-900">
        </div>
        <div class="w-3/12 pl-2 relative flex flex-col mr-2">
            <input type="number"
                   v-model="surcharge.sum"
                   required
        ...
        <button
            v-else-if="removable"
            @click.prevent="$emit('remove-surcharge', number)"
...
export default {
    ...
    computed: {
        removable() {
            return this.surcharges.length > 1
        },
        increasable() {
            return !this.number
        },
